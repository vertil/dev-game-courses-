...
class engine{
    ...
    glm::mat4 pasos=glm::mat4(1.0f);
    //-----
    //sound_buffer_wav * bdish;
    //------------------------------
    char active='n';
...
public:
    engine(){
        ...
        const int init_result = SDL_Init(SDL_INIT_EVERYTHING | SDL_INIT_AUDIO);
        if (init_result != 0)
        {
    ...
    void triagInit(){
        ...
        gMusic=Mix_LoadMUS("materials/songmp.mp3");
        if(gMusic==NULL){
            printf("Mix_LoadMUS(\"song.wav\"): %s\n", Mix_GetError());
            std::cout<<"can't load music"<<std::endl;
        }
        gMusic2=Mix_LoadMUS("materials/songmp.mp3");
        if(gMusic==NULL){
            printf("Mix_LoadMUS(\"song.wav\"): %s\n", Mix_GetError());
            std::cout<<"can't load music"<<std::endl;
        }
        bdish=Mix_LoadWAV("materials/bdish.wav");
        if(bdish==NULL){
            printf("Mix_LoadWAV: %s\n", Mix_GetError());
            std::cout<<"can't load bdish"<<std::endl;
        }
        //----
        body1.v[0].x=-0.15;
        body1.v[0].y=-0.15;
    ...
    bool load_texture(std::string_view path,uint number){
        ...
        glTexImage2D(GL_TEXTURE_2D, // type of texture
                     ...
                     static_cast<GLsizeif>(w),//texture weight
        ...
        while(SDL_PollEvent(&event_log)){
            if(event_log.type==SDL_QUIT){
                ...
                case SDL_SCANCODE_W:
                    ...
                    bodyMove=glm::translate(bodyMove, glm::vec3(0.0f,moving,0.0f));
                    pasos*=bodyMove;
    ...
    std::string activateProgBody(){
        ...
        glUniformMatrix4fv(mem,1,GL_FALSE,glm::value_ptr(pasos));
                OM_GL_CHECK()
